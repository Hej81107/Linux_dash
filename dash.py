...
# CPU Figure
fig_cpu = Figure(figsize=(4, 2))  # no explicit color or style
ax_cpu = fig_cpu.add_subplot()    # Single plot in this figure
line_cpu, = ax_cpu.plot([], [])   # Initialize an empty line
...
def update_plots():
    ...
    # Adjust y-limits dynamically or set a fixed range
    if cpu_data:
    ...
    if io_data:
        ax_io.set_ylim(0, max(io_data) * 1.1)  # or add some margin
    canvas_io.draw()
